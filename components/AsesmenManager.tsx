...
const AsesmenManager: React.FC<AsesmenManagerProps> = ({ type, user }) => {
  ...
  const renderSoalContent = (content: string, item: KisiKisiItem, isPrint = false, isStimulus = false) => {
    ...
    let preprocessed = content.replace(/([A-D][\.\)])\s/g, '\n$1 ').replace(/(\S)\s*\[\s*\]/g, '$1\n[]').replace(/\[\s?\]/g, 'CHECKBOX_MARKER');
        ...
        renderedParts.push(
          <div key={key} className={`whitespace-pre-wrap text-justify leading-relaxed ${item.bentukSoal === 'Isian' ? (isPrint ? 'mb-8' : 'mb-6') : 'mb-4'}`}>
            {currentParagraphLines.map((line, li) => {
              ...
              if (isMultipleAnswer && (line.includes('CHECKBOX_MARKER') || trimmedLine.startsWith('[]'))) {
                return (
                  <div key={li} className="flex items-start gap-4 mb-3 pl-1">
                    <div className={`shrink-0 border-2 border-black rounded-sm ${isPrint ? 'w-4 h-4' : 'w-5 h-5'} bg-white`}></div>
                    <span className={`${isPrint ? 'text-[11px]' : 'text-[13px]'} font-black flex-1`}>{line.replace(/CHECKBOX_MARKER|\[\]/g, '').trim()}</span>
                  </div>
                );
              }
              return <div key={li} className="flex items-start gap-2 mb-2"><span className={`${isPrint ? 'text-[11px]' : 'text-[14px]'} leading-relaxed font-black`}>{line.replace(/CHECKBOX_MARKER|\[\]/g, '').trim()}</span></div>;
            })}
          </div>
        );
    ...
    const flushTable = (key: string) => {
      if (currentTableRows.length > 0) {
        ...
        if (isMatching) {
            ...
            if (firstRowJoined.includes('pernyataan') || firstRowJoined.includes('pilihan')) dataRows = rows.slice(1);
            ...
            dataRows = Array.from({ length: maxLength }, (_, i) => [shuffledLeft[i] || '', '', shuffledRight[i] || '']);
        }
        renderedParts.push(
          <div key={key} className={`my-8 ${isMatching ? 'w-full' : 'overflow-x-auto rounded-[1rem] border-2 border-black shadow-sm overflow-hidden'}`}>
            {isMatching ? (
              <div className="space-y-4">
                {dataRows.map((row, ri) => (
                  <div key={ri} className="grid grid-cols-[1fr,80px,1fr] gap-4 items-center">
                    <div className="relative border-2 border-rose-600 rounded-2xl px-5 py-4 font-black text-center bg-white min-h-[4rem] flex items-center justify-center">
                      <span className={isPrint ? 'text-[10px]' : 'text-[12px]'}>{row[0]}</span>
                      <div className="absolute top-1/2 -right-2 -translate-y-1/2 w-4 h-4 bg-rose-600 rounded-full border-2 border-white"></div>
                    </div>
                    <div className="flex items-center justify-center h-full relative"><div className="w-full h-px border-t border-dashed border-slate-300"></div></div>
                    <div className="relative border-2 border-indigo-600 rounded-2xl px-5 py-4 font-black text-center bg-white min-h-[4rem] flex items-center justify-center">
                      <div className="absolute top-1/2 -left-2 -translate-y-1/2 w-4 h-4 bg-indigo-600 rounded-full border-2 border-white"></div>
              ...
              <table className={`border-collapse w-full ${isPrint ? 'text-[10px]' : 'text-[12px]'} table-fixed`}>
                <colgroup>{isGrid && <><col style={{ width: '60%' }} /><col style={{ width: '20%' }} /><col style={{ width: '20%' }} /></>}</colgroup>
                <thead><tr className="bg-slate-100">{rows[0].map((cell, i) => (<th key={i} className="border-2 border-black p-3 font-black text-center uppercase tracking-tight text-[11px]">{cell}</th>))}</tr></thead>
                <tbody>{rows.slice(1).map((row, ri) => (<tr key={ri} className="hover:bg-slate-50/50 transition-colors">{row.map((cell, ci) => (<td key={ci} className={`border-2 border-black p-3 ${isGrid && ci > 0 ? 'text-center' : 'text-justify'} align-middle`}>{isGrid && ci > 0 ? (<div className="flex justify-center items-center py-2"><div className={`${isPrint ? 'w-5 h-5' : 'w-6 h-6'} border-2 border-black rounded-md bg-white shadow-sm`}></div></div>) : (<div className="min-h-[1.5em] font-black">{cell}</div>)}</td>))}</tr>))}</tbody>
              </table>
            )}
          </div>
        );
        ...
        <div ref={printRef} className={`${isKisiKisi ? 'max-w-[29.7cm]' : 'max-w-[21cm]'} mx-auto bg-white p-4`}>
           ...
           {isKisiKisi ? (
              <table className="w-full border-collapse border-2 border-black text-[9pt]">
                ...
                <tbody>{filteredKisikisi.map((item) => (<tr key={item.id} className="break-inside-avoid align-top"><td className="p-3 border border-black"><p className="font-black uppercase mb-1 leading-tight">{item.elemen}</p><p className="text-[8pt] italic text-slate-600 leading-tight">{item.cp}</p></td><td className="p-3 border border-black text-center font-bold">{item.kompetensi === 'Pengetahuan dan Pemahaman' ? 'L1' : item.kompetensi === 'Aplikasi' ? 'L2' : 'L3'}</td><td className="p-3 border border-black text-justify leading-tight font-black">{item.indikatorSoal}</td><td className="p-3 border border-black text-center uppercase text-[8pt] font-bold">{item.bentukSoal}</td><td className="p-3 border border-black">{item.stimulus && (<div className="mb-4 p-2 bg-slate-50 border border-slate-200 text-[8.5pt] italic font-black">{renderSoalContent(item.stimulus, item, true, true)}</div>)}<div className="leading-relaxed">{renderSoalContent(item.soal, item, true, false)}</div></td><td className="p-3 border border-black text-center font-black">{item.kunciJawaban}</td><td className="p-3 border border-black text-center font-black">{item.nomorSoal}</td></tr>))}</tbody>
              </table>
           ) : (
           ...
           <div className="mt-16 grid grid-cols-2 text-center text-[10pt] font-black uppercase break-inside-avoid"><div><p>Mengetahui,</p><p>{isKisiKisi ? 'Kepala Sekolah' : 'Orang Tua / Wali'}</p><div className="h-20"></div><p className={isKisiKisi ? 'border-b border-black inline-block min-w-[150px]' : ''}>{isKisiKisi ? settings.principalName : '( .................................... )'}</p></div><div><p>Bilato, ........................</p><p>Guru Mata Pelajaran</p><div className="h-20"></div><p className="border-b border-black inline-block min-w-[150px]">{user.name}</p></div></div>
        </div>
      </div>
    );
  }
